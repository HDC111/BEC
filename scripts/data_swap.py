...
def swap_dataset():
    ...
    from_table = "FROM public.marked_records"
    to_table = "FROM public.marked"
    updated_filters = 0
    for sid in slice_ids:
    ...
    for sid in slice_ids:
        ...
        changed = False
        if old_params and "marked_records" in old_params:
            old_params = old_params.replace("marked_records", target_table)
            changed = True
        if ds_type != "table":
