...
def swap_dataset(from_table_name, target_table):
    ...
    from_table = f"FROM public.{from_table_name}"
    to_table = f"FROM public.{target_table}"
    ...
    for sid in slice_ids:
        ...
        changed = False
        if old_params and from_table_name in old_params:
            old_params = old_params.replace(from_table_name, target_table)
            changed = True
        if ds_type != "table":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Swap dataset for a Superset dashboard.")
    parser.add_argument("from_table_name", help="Original dataset table name (e.g. case_20250717_marked_records)")
    parser.add_argument("target_table", help="New dataset table name (e.g. marked)")

    args = parser.parse_args()
    swap_dataset(args.from_table_name, args.target_table)
